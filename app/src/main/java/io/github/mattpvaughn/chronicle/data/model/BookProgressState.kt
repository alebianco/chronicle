...
/**
 * What a book's progress indicator should show — the decision, with no view attached (cu-198).
 *
 * Extracted from `bindProgressIndicators`, which is `View`/`ProgressBar`-typed and so unreachable
 * from Compose. As the screens migrate one at a time (cu-188) the two renderers coexist for months,
 * and re-implementing this branching on the Compose side is how they would drift — on a rule that
 * was itself a bug fix (cu-86), which is the worst kind to get subtly wrong.
 *
 * Three states, not two booleans, because the interesting case is the one cu-86 found: a book
 ...
 * ordering is not what protects cu-86, `isCompleted()`'s own `viewCount` check is. Verified by
 ...
 * — a book that has never been opened looking finished (cu-86's `a book with no duration does not
 * render as finished`).
 */
