 ...
 *
 * Passed in rather than resolved here: this function is called from a `RecyclerView`
 * binder, so it used to reach `Injector.get().plexConfig()` **on every bind** — in a hot render
 * path the whole per-tick Room invalidation fix was about. A function rather than the
 * `PlexConfig` itself keeps the adapters that call it from knowing there is a Plex.
 */
