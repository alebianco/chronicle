 ...
 * avoid.
 *
 * This is the whole of the View island. `CastMenu` — the `Menu`-based equivalent that this
 * replaces — went with `audiobook_details_menu.xml`; `MediaRouteButtonFactory` has a public
 * overload taking a bare [MediaRouteButton], so no `Menu` is involved.
 *
