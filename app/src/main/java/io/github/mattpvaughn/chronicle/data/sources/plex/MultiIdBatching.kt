...
/**
 * How many characters of comma-joined ids one multi-id request may carry (cu-156).
 *
 * **Deliberately far below what the server tolerates.** cu-150 probed a real Plex with a 22 KB URL
 * and got a 200, so Plex itself is not the constraint — but the relay is one of the three
 * connection tiers (cu-11), and a reverse proxy commonly caps the request line at 8 KB. 2 KB of
 ...
 *
 * Bounded by **length, not count**, because ids are free-form strings since cu-71 — a backend with
