 ...
 *
 * Qualified since the Hilt migration. The hand-written graph had **separate components**, so an
 * unqualified `CoroutineScope` was unambiguous — the service component saw the service's,
 * everything else saw the application's. Hilt puts them in one hierarchy, where the same type
 * bound twice is an error rather than a silent choice. Naming which scope is wanted is the point:
 * before, the answer
