 ...
 * code.
 *
 * The `plexGenres`-wrong-`@Json`-name bug is the reason this exists: it carried the wrong `@Json` name for the life of the
 * project while every hand-written fixture agreed with the code. A later bug then repeated the shape in
...
class RealTitleSortCorpusTest {
  ...
  fun `the built-in patterns read a series index from almost every real titleSort`() {
    ...
    // 138/139 on the captured library, up from 136 before `audnexus_subseries` was added. Pinned
  ...
  fun `the audnexus format is the dominant real-world shape`() {
    // 111 of 139 -- this is why `audnexus` is tried before `label-first`.
