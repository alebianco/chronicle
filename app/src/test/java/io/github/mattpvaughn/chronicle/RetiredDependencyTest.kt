 ...
 * back. Four of the five entries are from the same programme — decision-24 moved the transport, and the
 ...
 *
 * **`play-services-oss-licenses`** is the fifth, and the odd one out in
 * that nothing was wrong with it as a library — it is a *distribution* problem. Decision-1 puts
 * sideload/F-Droid/homelab first, F-Droid does not accept a Google Play Services dependency, and
 * the app's licences page was being generated by a tool that could not ship where the app ships.
 * The guard names the `.oss` package rather than the `com.google.android.gms` group, because the
 * Cast SDK is a deliberate, recorded exception (decision-19) confined to `CastPlayerProvider`.
 *
...
class RetiredDependencyTest {
  ...
  @Test
  fun `no source imports the Play Services licences library`() {
    assertEquals(
      "play-services-oss-licenses is a Google Play Services dependency, and decision-1 puts " +
        "sideload/F-Droid/homelab distribution first \u2014 F-Droid does not accept a GMS " +
        "dependency, so the tool nominally generating the licences page was itself a " +
        "distribution blocker. AboutLibraries generates it now; render the catalogue " +
        "through LicenseCatalogSource. Note this bans the licences package specifically, not " +
        "com.google.android.gms: the Cast SDK is an accepted exception (decision-19) and lives " +
        "behind CastPlayerProvider.",
      emptyList<String>(),
      filesImporting("com.google.android.gms.oss"),
    )
  }
