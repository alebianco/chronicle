...
Those version pins are **released**: the project moved to **AGP 9.4.0** on 2026-09-08, so the
Compose BOM is 2026.08.00, `lifecycle` 2.11.0 and `navigation-compose` 2.10.0. The gate was AGP
throughout, never compileSdk — 37 shipped on AGP 8.13.2 and lifted nothing, and
`checkDebugAarMetadata` named AGP for every held artifact and compileSdk for none.
